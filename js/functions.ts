...
export function CheckWinner(obj: Card[]): boolean {
    let result = false;
...
export function randomIconGenerator(length: number): string[] {
    let tempLen = length / 2
    let randomIconArr: string[] = []
...
export function doubleRandomGenerator<T>(arr: T[]): T[] {
    const result = arr.reduce<T[]>((acc, item) => {
        acc.push(item, item);
        return acc;
    }, []);
...
export function mixingFunction<T>(arr: T[]): T[] {
...
export function nextPlayer(): void {
    ...
    if (activeIconIndex === -1) return; // agar hech biri faollashtirilmagan bo'lsa, chiqib ket
